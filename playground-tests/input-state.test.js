...
/** @type {import("../pkg/cli/playground/types").PlaygroundCapabilities} */
const capabilities = {
...
test("connect builds defaults without losing false, null, or typed enums", () => {
  const state = new InputState();
  state.connect("http://model", /** @type {any} */ (document), capabilities);
  assert.deepEqual(state.input, { choice: 1, enabled: false, text: "hello", nullable: null });
...
test("worker validation issues remain field-oriented and block running", async () => {
  const issues = [{ field: "text", keyword: "pattern", message: "Bad text", path: "input.text" }];
  await withWorker(
    () => issues,
    async () => {
      const state = connectedInput();
...
/** @param {()=>unknown[]|Promise<unknown[]>} validate @param {()=>Promise<void>} run @param {{onTerminate?:()=>void,setTimeout?:(callback:TimerHandler,delay?:number)=>number}} [options] */
async function withWorker(validate, run, options = {}) {
  const windowDescriptor = Object.getOwnPropertyDescriptor(globalThis, "window");
  const workerDescriptor = Object.getOwnPropertyDescriptor(globalThis, "Worker");
  class FakeWorker {
    /** @type {((event:{data:{id:number,issues:unknown[]}})=>void) | null} */ onmessage = null;
    /** @type {((event:Event)=>void) | null} */ onerror = null;
    postMessage(/** @type {{id:number}} */ message) {
      void Promise.resolve(validate()).then((issues) =>
        this.onmessage?.({ data: { id: message.id, issues } }),
      );
    }
    terminate() {
      options.onTerminate?.();
...
/** @param {"window"|"Worker"} name @param {PropertyDescriptor|undefined} descriptor */

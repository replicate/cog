...
import {
  ...
  FileReadGuard,
  MAX_LOCAL_FILE_SIZE,
} from "../pkg/cli/playground/lib/input/files.js";
import {
  dataMediaKind,
  emptyInputValue,
  formatBytes,
  isDataURI,
} from "../pkg/cli/playground/lib/input/input.js";

test("rejects local files larger than 16 MiB before reading", async () => {
  await assert.rejects(
    fileToDataURI(/** @type {File} */ ({ size: MAX_LOCAL_FILE_SIZE + 1 })),
...
test("resolves a successfully read file data URI", async () => {
  await withFileReader({ result: "data:image/png;base64,YQ==" }, async () => {
    assert.equal(
      await fileToDataURI(/** @type {File} */ ({ size: 1 })),
...
test("propagates browser file read errors", async () => {
  ...
  await withFileReader({ error: failure }, async () => {
    await assert.rejects(fileToDataURI(/** @type {File} */ ({ size: 1 })), failure);
  });
});

test("rejects non-string FileReader results", async () => {
  await withFileReader({ result: new ArrayBuffer(1) }, async () => {
    await assert.rejects(fileToDataURI(/** @type {File} */ ({ size: 1 })), /Could not read file/);
  });
});
...
/** @param {{result?:string|ArrayBuffer,error?:Error}} outcome @param {()=>Promise<void>} run */
async function withFileReader(outcome, run) {
  const descriptor = Object.getOwnPropertyDescriptor(globalThis, "FileReader");
  class FakeFileReader {
    result = outcome.result ?? null;
    error = outcome.error ?? null;
    /** @type {((event:Event)=>void) | null} */ onload = null;
    /** @type {((event:Event)=>void) | null} */ onerror = null;
    readAsDataURL() {
      queueMicrotask(() =>
        this.error ? this.onerror?.(new Event("error")) : this.onload?.(new Event("load")),
      );
    }
  }

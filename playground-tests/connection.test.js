...
import { ConnectionState } from "../pkg/cli/playground/lib/state/connection-state.js";
...
test("loads config and connects to its default target", async () => {
  await withWindow(async () => {
    ...
    const state = new ConnectionState(/** @type {any} */ (api));
    await state.start();
    await settle();
...
/** @param {{config?:Record<string,string>,configError?:Error,configPromise?:Promise<unknown>,schema?:(target:string)=>Promise<unknown>,schemaError?:Error,health?:(target:string)=>Promise<unknown>,healthError?:Error}} options */

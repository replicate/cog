...
`run()` can return strings, numbers, [`cog.Path`](#path) objects representing files on disk, or lists or dicts of those types. You can also define a custom [`Output()`](#outputbasemodel) for more complex return types.
...
If you have an [async `run()` method](#async-predictors-and-concurrency), you must use `cog.AsyncIterator` instead:

```py
from cog import AsyncIterator, BaseRunner, Path
...
When a prediction is canceled (via the [cancel HTTP endpoint](http.md#post-predictionsprediction_idcancel) or a dropped connection), the Cog runtime interrupts the running `run()` function. The exception raised depends on whether the predictor is sync or async:
...
Each parameter of the `run()` method must be annotated with a type. The method's return type must also be annotated. The supported types are:
...
```python
class Runner(BaseRunner):
    def run(self, prompt: str=Input(description="prompt", default=None)) -> str:

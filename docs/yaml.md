...
It has these top-level keys: [`build`](#build), [`image`](#image), [`predict`](#predict), and [`weights`](#weights). It looks a bit like this:
...
Declares model weights that are managed separately from the application image. Each entry names a weight set, declares where it should be mounted inside the container, and optionally specifies a source to import from.

Weights require the [`image`](#image) field to be set.
...
A unique identifier for the weight set. Must contain only lowercase alphanumeric characters, hyphens, dots, or underscores (e.g. `my-model-weights`). Names become registry path components, so they follow OCI naming rules.

### `weights[].target`

The absolute path where the weights are mounted inside the container (e.g. `/src/weights`). Targets must be unique and non-overlapping across weight entries.

### `weights[].source`

An optional object describing where to import weights from. If omitted, weights must be managed manually.

### `weights[].source.uri`

The source location for the weights. Supported schemes:

- `hf://org/repo` — Hugging Face Hub repository
- `file://./local/path` — local directory (relative to the project root)
- `/absolute/path` or `./relative/path` — bare paths (treated as `file://`)

### `weights[].source.include`

A list of glob patterns. When set, only files matching at least one pattern are imported. If omitted or empty, all files are included (subject to `exclude`).

### `weights[].source.exclude`

A list of glob patterns. Files matching any pattern are excluded from import, even if they also match an `include` pattern. Exclude always takes precedence.

### Pattern matching

Patterns use gitignore-style glob syntax:

- `*.safetensors` — matches any `.safetensors` file at any depth
- `onnx/*` — matches direct children of the `onnx/` directory
- `**/config.json` — matches `config.json` at any depth
- `original/` — matches the entire `original/` directory tree

Patterns use forward slashes on all platforms. Matching is case-sensitive. Pattern order within a list does not matter.

Empty-string patterns and `!`-prefixed negation patterns are rejected at config validation time.

**Compatibility with Hugging Face:** Common patterns like `*.safetensors`, `*.bin`, and `onnx/*` behave the same as HF's `allow_patterns`/`ignore_patterns`. One difference: `*` does not cross directory separators (gitignore semantics), so `onnx/*.bin` only matches direct children of `onnx/`, not deeper nesting. Use `onnx/**/*.bin` for recursive matching.

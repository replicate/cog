...
> [!NOTE]
> The `predict:` key still works for backwards compatibility, but `run:` is preferred for new models. For example, `predict: "predict.py:Predictor"` is equivalent to `run: "run.py:Runner"`.

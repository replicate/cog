...
def get_site_packages_bin_path() -> str:
    ...
    """
    site_packages_path = os.path.join(os.path.dirname(next(p for p in sys.path if 'site-packages' in p)),
                                      'site-packages/bin')
    exec_prefix_path = os.path.join(sys.exec_prefix, 'bin')
    bin_path = exec_prefix_path if exec_prefix_path != "/usr/local/bin" else site_packages_path
    return bin_path

...
class Path(pathlib.PosixPath):
    ...
    def validate(cls, value: Any) -> pathlib.Path:
        ...
        # this might be the best place to convert,
        # as long as you're converting to tempfile paths

        # this is also where you would need to note which tempfiles need to be filled
        if parsed_url.scheme == "data":
...
# we would prefer URLFile to stay lazy, but that doesn't really work with httpx
...
def get_filename_from_urlopen(resp: urllib.response.addinfourl) -> str:  # type: ignore

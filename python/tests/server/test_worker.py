...
async def _process(events, swallow_exceptions=False):
    """
    ...
    stdout = []
    stderr = []
    if not inspect.isasyncgen(events):
        events = make_generator_async(events)
    try:
...
class WorkerState(RuleBasedStateMachine):
    ...
    async def _check_setup_events(self):
        ...
        result = await _process(self.setup_events)
    ...
    async def _check_predict_events(self):
        ...
        result = await _process(self.predict_events)

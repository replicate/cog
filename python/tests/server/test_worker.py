...
async def _process_list(events: "list[_PublicEventType]") -> Result:
    async def make_list_async(it: Iterator) -> AsyncIterator:
        for item in it:
            yield item

    return await _process(make_list_async(events))


async def _process(events, swallow_exceptions=False) -> Result:
    """
...
async def test_cancel_is_safe():
    ...
    try:
        ...
        result1 = await _process(w.predict({"sleep": 0.5}), swallow_exceptions=True)

        for _ in range(50):
            w.cancel()

        result2 = await _process(w.predict({"sleep": 0.1}), swallow_exceptions=True)
...
class WorkerState(RuleBasedStateMachine):
    ...
    def read_setup_events(self, n):
        try:
            ...
            self.setup_generator = None
            self.await_(self._check_setup_events())

    async def _check_setup_events(self):
        assert isinstance(self.setup_events[-1], Done)

        result = await _process_list(self.setup_events)
    ...
    def read_predict_events(self, n):
        try:
            ...
            self.predict_generator = None
            self.await_(self._check_predict_events())
        except InvalidStateException:
            pass

    async def _check_predict_events(self):
        assert isinstance(self.predict_events[-1], Done)

        payload = self.predict_payload
        result = await _process_list(self.predict_events)

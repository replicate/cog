...
export class InputView {
  /** @param {HTMLElement} root @param {import("../lib/state/input-state.js").InputState} state @param {import("../lib/state/connection-state.js").ConnectionState} connection */
  constructor(root, state, connection) {
    this.root = root;
    this.state = state;
    ...
    /** @type {Map<string, {raw:string,error:string}>} */ this.structuredDrafts = new Map();
    /** @type {Map<string, {error:string,fileName:string}>} */ this.fileDrafts = new Map();
    /** @type {Map<string, {editor:JsonEditor,described:string|undefined,errorId:string}>} */
  ...
  render() {
    ...
    const head = element(
      ...
      segmentedTabs(
        ...
        this.state.mode,
        (mode) => this.state.changeMode(/** @type {"form"|"json"} */ (mode)),
      ),
    );
    section.append(head);
  ...
  renderJSON() {
    ...
    this.jsonEditor = new JsonEditor(host, {
      ...
      invalid: Boolean(this.state.jsonError) || Boolean(this.state.issues.length),
      describedBy: this.describedBy(),
      onChange: (value) => this.state.changeJSON(value),
    });
  ...
  /** @param {import("../types").OpenAPIDocument} document @param {import("../types").OpenAPISchema} schema */
  renderForm(document, schema) {
    /** @type {[string, import("../types").OpenAPISchema][]} */
    const properties = orderedProperties(schema).map(([name, value]) => [
      name,
  ...
  /** @param {import("../types").OpenAPIDocument} document @param {import("../types").OpenAPISchema} parent @param {string} name @param {import("../types").OpenAPISchema} schema @param {boolean} required */
  renderField(document, parent, name, schema, required) {
    ...
    if (!required) {
      ...
      checkbox.addEventListener("change", () => {
        ...
        if (checkbox.checked) {
          ...
          next[name] = Object.hasOwn(effective, "default")
            ? /** @type {import("../types").JsonValue} */ (effective.default)
            : (enumValues(document, effective)?.[0] ?? emptyInputValue(effective));
        } else {
          delete next[name];
  ...
  /** @param {import("../types").OpenAPIDocument} document @param {string} name @param {import("../types").OpenAPISchema} schema @param {unknown} value @param {boolean} required @param {boolean} disabled @param {import("../types").ValidationIssue[]} errors */
  renderControl(document, name, schema, value, required, disabled, errors) {
    const described =
      [
        ...
        .filter(Boolean)
        .join(" ") || undefined;
    const set = (/** @type {unknown} */ next) => {
      if (next !== undefined)
        this.state.changeForm(
          /** @type {import("../types").JsonObject} */ ({ ...this.state.input, [name]: next }),
        );
    };
    const common = {
      id: `field-${name}`,
  ...
  /** @param {string} name @param {import("../types").OpenAPISchema} schema @param {unknown} value @param {boolean} disabled @param {import("../types").ValidationIssue[]} errors @param {(value:unknown)=>void} set @param {string|undefined} described */
  renderFile(name, schema, value, disabled, errors, set, described) {

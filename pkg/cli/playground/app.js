...
const WEBHOOK_EVENTS = ["start", "output", "logs"];
...
export class App {
  ...
  constructor(root) {
    ...
    /** @type {import("./types").RunMode} */ this.runMode = "sync";
    this.predictionId = "";
    this.webhookEvents = new Set(WEBHOOK_EVENTS);
    /** @type {import("./types").PlaygroundCapabilities | undefined} */ this.lastCapabilities =
      undefined;
    /** @type {string | undefined} */ this.schemaURL = undefined;
  ...
  renderToolbar() {
    ...
    target.id = "target";
    const modes = element("fieldset", { className: "playground-options" });
    modes.append(element("legend", { className: "sr-only", text: "Prediction mode" }));
    for (const [value, label] of [
      ["sync", "Sync"],
      ...(capabilities?.streaming ? [["stream", "Stream"]] : []),
      ...(capabilities?.async ? [["async", "Async"]] : []),
    ])
      modes.append(
        element("button", {
          id: `run-mode-${value}`,
          ...
          disabled: controlsDisabled,
          onclick: () => {
            this.runMode = /** @type {import("./types").RunMode} */ (value);
            this.renderToolbar();
          },
        }),
      );
    ...
    const runnable = Boolean(
      capabilities &&
      !this.input.formBusy &&
      !this.input.validating &&
      !this.prediction.running &&
      (this.runMode !== "async" || this.connection.webhookBase),
    );
    ...
    const actionBar = element(
      ...
      capabilities?.streaming || capabilities?.async ? modes : undefined,
      element(
        "label",
  ...
  renderWebhookOptions() {
    ...
    for (const event of WEBHOOK_EVENTS) {
      const checkbox = element("input", {
        ...
        disabled: disabled || event === "completed",
        "aria-disabled": event === "completed" ? "true" : undefined,
      });
  ...
  async run() {
    const capabilities = this.connection.capabilities;
    if (!capabilities || this.prediction.running || this.input.formBusy || this.input.validating)
      return;
    ...
    void this.prediction.run({
      ...
      webhookEvents: /** @type {import("./types").WebhookEvent[]} */ ([...this.webhookEvents]),
    });
  }
  openSchema() {

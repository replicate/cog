...
/** @typedef {{token:string,target:string,endpoint:string,predictionId?:string,abort:AbortController,startedAt:number,events?:EventSource,webhookReceived:boolean}} ActiveRun */
/** @typedef {{rawEvents:string[],rawLength:number,output:unknown[],outputLength:number}} StreamBuffer */
...
export class PredictionState extends Store {
  ...
  constructor(api) {
    ...
    this.api = api;
    this.running = false;
    /** @type {import("../../types").PredictionEnvelope | undefined} */ this.envelope = undefined;
    /** @type {unknown} */ this.output = undefined;
    /** @type {import("../../types").RunMode | undefined} */ this.mode = undefined;
    /** @type {string[]} */ this.rawEvents = [];
    this.error = "";
    /** @type {import("../../types").RequestTrace | undefined} */ this.trace = undefined;
    /** @type {ActiveRun | undefined} */ this.active = undefined;
    /** @type {string | undefined} */ this.traceToken = undefined;
    /** @type {StreamBuffer} */ this.buffer = emptyBuffer();
    this.destroyed = false;
  }

  /** @param {{target:string, endpoint:string, predictionId?:string, input:import("../../types").JsonObject, mode:import("../../types").RunMode, webhookBase:string, webhookEvents:import("../../types").WebhookEvent[]}} options */
  async run(options) {
  ...
  /** @param {string} token @param {any} options @param {AbortSignal} signal */
  async runSync(token, options, signal) {
    const response = await this.api.submit({
      ...options,
      id: options.predictionId,
      signal,
      onResponse: (value) => this.captureResponse(token, value),
    });
    if (this.active?.token !== token) return;
  ...
  /** @param {string} token @param {any} options @param {AbortSignal} signal */
  async runStreaming(token, options, signal) {
    /** @type {string[]} */ let rawFrames = [];
    ...
    for await (const event of this.api.stream({
      ...
      id: options.predictionId,
      signal,
      onResponse: (value) => this.captureResponse(token, value, "SSE connection opened"),
    })) {
      if (this.active?.token !== token) return;
  ...
  /** @param {string} token @param {any} options @param {AbortSignal} signal */
  async runAsync(token, options, signal) {
    ...
    events.onmessage = (/** @type {MessageEvent<string>} */ event) => {
      if (this.active?.token !== token) return;
  ...
  /** @param {string} token @param {string} raw @param {unknown} [nextOutput] */
  queueStream(token, raw, nextOutput) {
    if (this.active?.token !== token) return;
  ...
  /** @param {string} token @param {any} options */
  ...
  /** @param {string} token @param {import("../../types").TraceEventKind} kind @param {string} label @param {unknown} [data] */
  record(token, kind, label, data) {
  ...
  /** @param {any} run @param {import("../../types").TraceEventKind} kind @param {string} label @param {unknown} [data] */
  appendFinished(run, kind, label, data) {
  ...
  /** @param {string} token @param {Response} response @param {string} [label] */
  captureResponse(token, response, label) {
  ...
  /** @param {string} token @param {unknown} body @param {boolean} [attach] */
  setTraceBody(token, body, attach = true) {
    if (this.active?.token !== token || !this.trace) return;
  ...
  /** @param {string} token @param {unknown} body */
  setTraceRequestBody(token, body) {
...
/** @param {unknown[]} items @param {number} length @param {number} maxItems @param {number} maxText */
function trim(items, length, maxItems, maxText) {
  while (items.length > 1 && (items.length > maxItems || length > maxText))
    length -= valueLength(items.shift());
  return length;
}
/** @param {Response} response */
export function modelResponseHeaders(response) {

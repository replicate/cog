...
func TestCachingZip(t *testing.T) {
	...
	require.NoError(t, err)
	requireUnzippedCorrectly(t, unzipDir1, "foo", "bar", "baz", "other")
	...
	require.NoError(t, err)
	requireUnzippedCorrectly(t, unzipDir2, "foo", "bar", "baz", "other")
	...
	requireUnzippedCorrectly(t, unzipDir3,
		...
		"changed-baz",
		"cogcached9298a10d1b0735837dc4bd85dac641b0f3cef27a47e5d53a54f2f3f5b2fcffa",
	)
	...
	require.NoError(t, err)
	requireUnzippedCorrectly(t, unzipDir4, "foo", "bar", "changed-baz", "other")
...
func requireUnzippedCorrectly(t *testing.T, dir string, foo string, bar string, baz string, other string) {
	...
	contents, err = os.ReadFile(filepath.Join(dir, "other-link.txt"))
	require.NoError(t, err)

...
func (g *Generator) Generate() (weightsBase string, dockerfile string, dockerignoreContents string, err error) {
	...
	base := []string{
		"# syntax = docker/dockerfile:1.2",
		fmt.Sprintf("FROM %s AS %s", config.DockerImageName(g.Dir)+"-weights", "weights"),
		"FROM " + baseImage,
		g.preamble(),
		g.installTini(),
		installPython,

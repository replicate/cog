...
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/lib/x86_64-linux-gnu:/usr/local/nvidia/lib64:/usr/local/nvidia/bin
ENV NVIDIA_DRIVER_CAPABILITIES=all
` + testTini() + testInstallPython("3.12") + "RUN ln -s `realpath \\`pyenv which python\\`` /python && chmod +x /python" + `
...
ENV NVIDIA_DRIVER_CAPABILITIES=all
` + testTini() +
		testInstallPython("3.12") + "RUN ln -s `realpath \\`pyenv which python\\`` /python && chmod +x /python" + `

...
// 	stage1, err := p.AddStage(plan.PhaseSystemDeps, "apt-install")
// 	require.NoError(t, err)
// 	stage1.Operations = []plan.Op{
// 		plan.Exec{Command: "apt-get update && apt-get install -y build-essential"},
// 	}

// 	stage2, err := p.AddStage(plan.PhaseSystemDeps, "tools-install")
// 	require.NoError(t, err)
...
// 	stage3, err := p.AddStage(plan.PhaseRuntime, "python-install")
// 	require.NoError(t, err)
...
// 	stage4, err := p.AddStage(plan.PhaseFrameworkDeps, "copy-phase")
// 	require.NoError(t, err)
...
// 				stage1, _ := p.AddStage(plan.PhaseSystemDeps, "stage1", plan.WithName("Stage 1"))
// 				stage1.Operations = []plan.Op{plan.Exec{Command: "echo hello"}}

// 				// Add a stage that references the phase
// 				stage2, _ := p.AddStage(plan.PhaseRuntime, "stage2", plan.WithName("Stage 2"))
...
// 				stage1, _ := p.AddStage(plan.PhaseRuntime, "stage1", plan.WithName("Stage 1"))
...
// 	// Add a stage to system-deps phase
// 	stage1, err := p.AddStage(plan.PhaseSystemDeps, "build-tools", plan.WithName("Build Tools"))
// 	require.NoError(t, err)
// 	stage1.Operations = []plan.Op{
...
// 	stage2, err := p.AddStage(plan.PhaseRuntime, "use-tools", plan.WithName("Use Tools"))
// 	require.NoError(t, err)
// 	stage2.Operations = []plan.Op{
// 		plan.Exec{

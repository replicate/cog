	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() *Plan {
				...
				stage1, _ := p.AddStage(PhaseSystemDeps, "stage1", "stage1")
				stage1.Operations = []Op{Exec{Command: "echo hello"}}

				// Add a stage that references the phase
				stage2, _ := p.AddStage(PhaseRuntime, "stage2", "stage2")
				stage2.Source = Input{Phase: PhaseSystemDeps}
		...
		},
		{
			...
			setup: func() *Plan {
				...
				stage1, _ := p.AddStage(PhaseRuntime, "stage1", "stage1")
				stage1.Source = Input{Phase: PhaseSystemDeps} // This phase has no stages
				stage1.Operations = []Op{Exec{Command: "echo hello"}}
...
func TestGetPhaseResult(t *testing.T) {
	...
	stage1, err := p.AddStage(PhaseSystemDeps, "stage1", "stage1")
	...
	stage2, err := p.AddStage(PhaseSystemDeps, "stage2", "stage2")
	require.NoError(t, err)
	stage2.Operations = []Op{Exec{Command: "echo world"}}
	...
	// Test export phase
	exportStage, err := p.AddStage(ExportPhaseRuntime, "export1", "export1")
	require.NoError(t, err)

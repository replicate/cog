...
// 	// Create a test plan composer
// 	composer := plan.NewPlanComposer()
// 	composer.SetPlatform(plan.Platform{OS: "linux", Arch: "amd64"})
...
// 	// Create a more realistic plan composer with existing phases
// 	composer := plan.NewPlanComposer()
// 	composer.SetPlatform(plan.Platform{OS: "linux", Arch: "amd64"})
// 	composer.SetBaseImage(&baseimg.BaseImage{
// 		Build:   "ubuntu:20.04",
...
// 	precedingStage, err := composer.AddStage(plan.PhaseFrameworkDeps, "framework-setup", plan.WithName("Framework Setup"))
// 	require.NoError(t, err)
...
// 	assert.Equal(t, "framework-setup", frameworkPhaseResult.ID)
// 	assert.Equal(t, frameworkPhaseResult.ID, cogWheelStage.Source.Stage)
// }

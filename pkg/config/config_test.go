...
func TestValidateModelPythonVersion(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			pythonVersion:  "3.10",
			concurrencyMax: 5,
			expectedErr:    "when concurrency.max is set, minimum supported Python version is 3.11. requested 3.10",
		},
		{
		...
		},
		{
			...
			pythonVersion: "3.7",
			expectedErr:   "minimum supported Python version is 3.10. requested 3.7",
		},
	}

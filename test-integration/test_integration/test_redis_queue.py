...
def test_queue_worker_yielding_returning(docker_network, docker_image, redis_client):
    project_dir = Path(__file__).parent / "fixtures/yielding-returning-project"
    subprocess.run(["cog", "build", "-t", docker_image], check=True, cwd=project_dir)
    ...
    with docker_run(
        ...
        response = json.loads(redis_client.brpop("response-queue", timeout=10)[1])
        assert response == {"value": ["foo", "bar", "baz", "qux"], "status": "processing"}

        response = json.loads(redis_client.brpop("response-queue", timeout=10)[1])
        assert response == {"value": ["foo", "bar", "baz", "qux"], "status": "success"}

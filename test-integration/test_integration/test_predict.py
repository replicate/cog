...
def test_predict_takes_string_inputs_and_returns_strings_to_stdout(cog_path):
    project_dir = Path(__file__).parent / "fixtures/string-project"
    result = subprocess.run(
        [cog_path, "predict", "-i", "world"],
        cwd=project_dir,
        check=True,
        capture_output=True,
    )
...
def test_predict_takes_int_inputs_and_returns_ints_to_stdout(cog_path):
    project_dir = Path(__file__).parent / "fixtures/int-project"
    result = subprocess.run(
        [cog_path, "predict", "-i", "2"],
        cwd=project_dir,
        check=True,
        capture_output=True,
    )
...
def test_predict_takes_file_inputs(cog_path, tmpdir_factory):
    ...
    with open(out_dir / "input.txt", "w") as fh:
        fh.write("what up")
    result = subprocess.run(
        [cog_path, "predict", "-i", "path=@" + str(out_dir / "input.txt")],
        cwd=out_dir,
        check=True,
        capture_output=True,
    )
...
def test_predict_writes_files_to_files(cog_path, tmpdir_factory):
...
def test_predict_writes_files_to_files_with_custom_name(cog_path, tmpdir_factory):
    project_dir = Path(__file__).parent / "fixtures/file-output-project"
    out_dir = pathlib.Path(tmpdir_factory.mktemp("project"))
    shutil.copytree(project_dir, out_dir, dirs_exist_ok=True)
    result = subprocess.run(
        [cog_path, "predict", "-o", out_dir / "myoutput.bmp"],
        cwd=out_dir,
        check=True,
        capture_output=True,
    )
...
def test_predict_writes_multiple_files_to_files(cog_path, tmpdir_factory):
...
def test_predict_writes_strings_to_files(cog_path, tmpdir_factory):
    project_dir = Path(__file__).parent / "fixtures/string-project"
    out_dir = pathlib.Path(tmpdir_factory.mktemp("project"))
    result = subprocess.run(
        [cog_path, "predict", "-i", "world", "-o", out_dir / "out.txt"],
        cwd=project_dir,
        check=True,
        capture_output=True,
    )
...
def test_predict_runs_an_existing_image(cog_path, tmpdir_factory):
    project_dir = Path(__file__).parent / "fixtures/string-project"
    image_name = "cog-test-" + random_string(10)

    try:
        subprocess.run(
            [cog_path, "build", "-t", image_name],
            cwd=project_dir,
            check=True,
        )
        ...
        result = subprocess.run(
            [cog_path, "predict", image_name, "-i", "world"],
...
def test_predict_with_remote_image(cog_path, tmpdir_factory):
    ...
    another_directory = tmpdir_factory.mktemp("project")
    result = subprocess.run(
        [cog_path, "predict", image_name, "-i", "text=world"],
        cwd=another_directory,
        check=True,
        capture_output=True,
    )
...
def test_predict_in_subdirectory_with_imports(cog_path, tmpdir_factory):
...
def test_predict_many_inputs(cog_path, tmpdir_factory):

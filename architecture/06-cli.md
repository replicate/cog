...
The proxy is intentionally user-directed: this is a local development tool for APIs selected by the user, not a remotely hosted gateway. The checked-in HTML, CSS, and JavaScript under `pkg/cli/playground/` are embedded directly in every Go binary. There is no frontend build in local or release workflows. Browser dependencies are loaded at runtime from pinned cdnjs and esm.sh URLs, while framework-independent core tests install matching npm dependencies. AJV's dynamic schema compilation is confined to a worker with a narrower content security policy than the page.
